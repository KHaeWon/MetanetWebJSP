...
<%
	...
	/*  
		...
		>>단점 : 배우기 어렵다 , 디자인 작업 (힘들어요 ) , 협업(x)
				...
				out.print("<html>");
					...
					out.print("</body>");
				out.print("</html>");	
		>> JSP(java server page, 서버에서 컴파일되고 실행되는 웹용 파일) 탄생 >> 장점 >> UI 작업 >> web page >> html base >> 디자인 good >> 단점 java코드 <% ...안에
		>> 혼재 (JAVA + UI) >> 힘들어....
	...
	분업 하자
	Model (DTO , DAO, service)  >> 순수한 JAVA
	View  (UI)         			>> JSP => el, jstl
    ...
    ex) 톰켓(아파치 톰켓 설치) >> 웹서버 + WAS >> 요청 >> http://www.it.co.kr/login.jsp >> 실행 파일 (없어요) >>
        ...
        D:\Duzon\Web\WebBack\WebLabs\.metadata\.plugins\org.eclipse.wst.server.core\tmp0\work\Catalina\localhost\WebJSP\org\apache\jsp
	
    	상식 퀴즈)
 		asp , aspx , php , jsp 공통점은 무엇일까 => 서버에서 컴파일되고 실행된다는 공통점
 		코드 노출이 안 돼요 >> 보안에 좋아요 >> 클라이언트는 실행 결과만 보아요 (text 또는 html 형태로)
