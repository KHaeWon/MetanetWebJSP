...
<%
	...
	*/
	...
	String uid = request.getParameter("userid");//2. 데이터 받기
	String pwd = request.getParameter("pwd");//2.
	
	//hobby > input name="hobby"
	//배열
	
	String[] hobbyarray = request.getParameterValues("hobby");//2.
	/*  
	한글처리
	전송방식
	...
	>>get방식으로 넘어오는 한글은 깨지지 않음, post는 깨짐
	
	post방식 (한글깨짐) : http://localhost:8090/WebJSP/Ex02_register.jsp
	>>별도의 한글 깨짐 방지 처리를 해주어야 한다.
	>>request 안에 자원에 (인코딩) 
